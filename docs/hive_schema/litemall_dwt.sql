...
-- dwt_date_topic
drop table if exists dwt_date_topic;
create external table dwt_date_topic(
...
STORED AS PARQUET
LOCATION '/warehouse/litemall/dwt/dwt_date_topic'
TBLPROPERTIES ('parquet.compression'='lzo');
...
STORED AS PARQUET
LOCATION '/warehouse/litemall/dwt/dwt_region_topic'
TBLPROPERTIES ('parquet.compression'='lzo');
...
create external table dwt_sku_topic(
	...
	`payment_30_days_num` bigint comment '近30天被支付件数',
	...
	`refund_amount` decimal(10,2) comment '累计被退单金额',
...
create external table dwt_groupon_topic(
	`id` int comment '团购ID',
	`groupon_name` string comment '团购名称',
	`order_count` bigint comment '累计下单次数',
