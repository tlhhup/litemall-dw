...
/**
  ...
  * @date 2021-07-28
  */
object OdsGoodsCategoryApp extends App {
  ...
  //2. kafka参数
  val topics = "ods_litemall_category"
  ...
  //3. 创建spark context
  val conf = new SparkConf().setMaster("local[*]").setAppName("OdsGoodsCategoryApp")
  ...
  offSetDs.map(item => parse(item.value()).extract[GoodsCategory])
    .foreachRDD(rdd => {
