...
/**
  ...
  * @date 2021-07-26
  */
object CanalOdsDispatcherApp extends App {
  ...
  //3. 创建spark context
  val conf = new SparkConf().setMaster("local[*]").setAppName("CanalOdsDispatcherApp")
        ...
        for (item <- iter) {
          ...
          (item \ "type").extract[String] match {
            case "INSERT" => {
              ...
              // 解析数据
              val data = write((item \ "data").extract[List[Map[String, Any]]])
